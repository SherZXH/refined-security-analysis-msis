...
def workout_bkz_tour(g6k, tracer, blocksize, dim4free_fun=theo_dim4free_fun2,
    ...
    '''
    dim4free =round(blocksize/log(blocksize)) +extra_dim4free
    '''
    ...
    if tours == 0:
        if blocksize == 12100:
            kappa_list =list(range(83,int((d-50-dim4free_max)/jump_kappa)*jump_kappa+1,jump_kappa)) # kappa must end on at least the d - 50 -f_max (f_max is in the workout)
        else:
    ...
    for kappa in kappa_list:
        ...
        for t in range(inner_tours):
            ...
            sys.stdout.flush()
            workout(g6k, tracer, kappa, beta, f_min,f_max,goal_r0=goal_r0,pump_params=pump_params, dim4free_dec=jump,current_beta = blocksize,extra_dim4free=extra_dim4free)
            g6k.lll(0, d)
...
def pump_n_jump_bkz_tour(g6k, tracer, blocksize,  jump=1,
    ...
    #dim4free = int((default_dim4free_fun(blocksize) + theo_dim4free_fun(blocksize))/2) + extra_dim4free
    r = [g6k.M.get_r(j, j) for j in range(d)]
    log_vol = sum([log(x) for x in r])
    log_vol_n = 1.0 / d * log_vol
    vol_n = sqrt(exp(log_vol_n))
    ...
    dim4free = theo_dim4free_fun2(blocksize,rhf) + extra_dim4free
    ...
    indices = []
    #indices  = [(0, blocksize - dim4free_max + i, i) for i in range(0, dim4free_max, jump)]

    #indices += [(i, blocksize, j) for j in range(dim4free_max,dim4free_min,-jump) for i in range(0, d - blocksize, jump)] #backup: overall dim4free_max -> dim4free_min

    indices += [(i, blocksize, j) for i in range(142, d - blocksize, jump) for j in range(dim4free_max,dim4free_min-jump,-jump)] #backup: overall dim4free_max -> dim4free_min

    indices += [(d - blocksize + i, blocksize - i, dim4free_max - i) for i in range(0, dim4free_max, jump)]


    print(indices)


    pump_params["down_stop"] = dim4free_max+3
    ...
    for (kappa, beta, f) in indices:
        ...
        if not os.path.isdir("qarychallenge/dim-%d/wnjbkz-%d-jump-%d" % (d,blocksize - extra_dim4free, jump)):
            os.mkdir("qarychallenge/dim-%d/wnjbkz-%d-jump-%d" % (d,blocksize - extra_dim4free, jump))

        if kappa >= 0:
            block_filename = "qarychallenge/dim-%d/wnjbkz-%d-jump-%d/wnjbkz-%d-jump-%d-kappa-%d-beta-%d-f-%d" % (d,blocksize - extra_dim4free, jump,blocksize - extra_dim4free, jump,kappa,beta,f)
