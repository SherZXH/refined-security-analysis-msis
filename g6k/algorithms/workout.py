...
#revised workout
def workout(g6k, tracer, rank,kappa, blocksize, dim4free_min=0,              # Main parameters
    ...
    with tracer.context(("workout", "kappa:%d beta:%d f:%d" % (kappa, blocksize, dim4free_max))):
        for f in fs:
            ...
            if rank ==0:
                ...
                if kappa == 0:
                    block_filename = "qarychallenge/dim-%d/wnjbkz-%d-jump-%d/wnjbkz-%d-jump-%d-kappa-%d-beta-%d-f-%d" % (g6k.full_n,current_beta - extra_dim4free,dim4free_dec,blocksize - extra_dim4free,dim4free_dec,kappa,blocksize-dim4free_max+f,f)
                    fn = open(block_filename, "w")
                    fn.write(str(g6k.M.B))
                    fn.close()
                else:
                    block_filename = "qarychallenge/dim-%d/wnjbkz-%d-jump-%d/wnjbkz-%d-jump-%d-kappa-%d-beta-%d-f-%d" % (g6k.full_n,current_beta - extra_dim4free,dim4free_dec,blocksize - extra_dim4free,dim4free_dec,kappa,blocksize,f)
                    fn = open(block_filename, "w")
                    fn.write(str(g6k.M.B))
                    fn.close()
            else:
                sub_dir = "qarychallenge/dim-%d/headwnjbkz-%d-jump-%d" % (g6k.full_n,blocksize - extra_dim4free,dim4free_dec)
                os.makedirs(sub_dir, exist_ok=True)

                if kappa == 0:
                    block_filename = "qarychallenge/dim-%d/headwnjbkz-%d-jump-%d/headwnjbkz-%d-jump-%d-kappa-%d-beta-%d-f-%d" % (g6k.full_n,current_beta - extra_dim4free,dim4free_dec,blocksize - extra_dim4free,dim4free_dec,kappa,blocksize-dim4free_max+f,f)
                    fn = open(block_filename, "w")
                    fn.write(str(g6k.M.B))
                    fn.close()
                else:
                    block_filename = "qarychallenge/dim-%d/headwnjbkz-%d-jump-%d/headwnjbkz-%d-jump-%d-kappa-%d-beta-%d-f-%d" % (g6k.full_n,current_beta - extra_dim4free,dim4free_dec,blocksize - extra_dim4free,dim4free_dec,kappa,blocksize,f)
                    fn = open(block_filename, "w")
                    fn.write(str(g6k.M.B))
                    fn.close()
