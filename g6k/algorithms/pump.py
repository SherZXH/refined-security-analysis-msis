...
#naive_bkz_tour(g6k,dummy_tracer,blocksize=beta, dim4free_fun=theo_dim4free_fun,extra_dim4free=12, workout_params=None, pump_params=pump_params, goal_r0=goal_r0)
...
def wrapped_sieve(pump):
    ...
    if pump.g6k.l - pump.l <= 3:
        ...
        # only dh_hash in top (pump_dim - bgj3_MSD) pumps
    """
    dh_dim4free = 0
    if pump.g6k.l - pump.l <= 21:
        dh_dim4free = min(pump.g6k.params.dh_dim4free, pump.g6k.l - pump.insert_left_bound)
    """
    ...
    if pump.goal_r0 is not None:
        ...
        if (pump.g6k.M.get_r(0, 0) <= pump.goal_r0): #or abs(pump.g6k.M.B[pump.kappa][-1]) == 1:
            cont = False
...
def scoring_down(i, nlen, olen, aux):
    if i < aux.insert_left_bound or nlen >= olen:
        return False
    #print(aux.prefer_left_insert)
    return log(olen / nlen) - i * log(aux.prefer_left_insert)


def pump(g6k, tracer, kappa, blocksize, dim4free, down_sieve=True, down_sat=None, # Main parameters
    ...
    #print("down_sieve=%s"%(pump.down_sieve))
    #print("saturation_error=%s"%(pump.saturation_error))
    max_RAM_cost = 0
    with tracer.context(("pump", "kappa:%d beta:%d f:%d" % (kappa, blocksize, dim4free))):
        with g6k.temp_params(reserved_n=pump.r-pump.l, goal_r0=goal_r0kappa):
            ...
            #print("db_lift_probability: %f ." %g6k.db_lift_probability())
            if (goal_r0 is not None and (g6k.M.get_r(0, 0) <= goal_r0) ):# or abs(g6k.M.B[kappa][-1]) == 1:
                print('Solution:', str(g6k.M.B[0]))
                return flast,max_RAM_cost
            
            T_pump_up = time.time() - T0
            
            #print("pump_up_time: %f sec." % T_pump_up)
            
            """
            pump_up_filename = "svpchallenge/pump-up-llb-%d-beta-%d-f-%d" % (kappa,blocksize,dim4free) #store after high-dimension pump processing lattice basis matrix step by step
            fn = open(pump_up_filename, "w")
                ...
                with tracer.context(("pump-step-down", "l:%d r:%d n:%d" % (g6k.l, g6k.r, g6k.n))):
                    with g6k.temp_params(saturation_ratio=pump.saturation_ratio_down):
                        ...
                        if not pump.down_sieve:
                            ...
                            #print_pump_state2(pump)
                        """
                        pump_down_sub_filename = "svpchallenge/pump-down-llb-%d-beta-%d-f-%d-%d-%d-%d"%(kappa,blocksize,dim4free,g6k.l,g6k.r,g6k.n)
                        fn = open(pump_down_sub_filename, "w")
                        fn.write(str(g6k.M.B))
                        fn.close()
                        """
                        
            """
            pump_down_filename = "qarychallenge/dim-340/pump-down-llb-%d-beta-%d-f-%d" % (kappa,blocksize,dim4free) #store after high-dimension pump processing lattice basis matrix step by step
            fn = open(pump_down_filename, "w")
            fn.write(str(g6k.M.B))
            fn.close()
            """
    ...
    #print("pump_time: %f sec." % (time.time() - T0))
